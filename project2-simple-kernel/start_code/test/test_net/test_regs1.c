...
{
    int i;
    memcpy((void*)0xa2f00000,&buffer[0],PSIZE*4);
    for(i=0;i<PNUM;i++){
        s_desc[i].tdes0 = 0x00000000;
        s_desc[i].tdes1 = 0x01000100;
        s_desc[i].tdes2 = (uint32_t)0x02f00000;
        s_desc[i].tdes3 = (uint32_t)&s_desc[i+1];
        if(i==(PNUM-1)){
            s_desc[i].tdes1 = 0x03000100;
            s_desc[i].tdes3 = (uint32_t)&s_desc[0];
        }
    }
...
static void recv_desc_init(mac_t *mac)
{
    ...
    for(i=0;i<PNUM;i++){
        ...
        r_desc[i].tdes1 = 0x01000100;
        r_desc[i].tdes2 = (uint32_t)&recv_buffer[i][0];
        r_desc[i].tdes3 = (uint32_t)&r_desc[i+1];
        if(i==(PNUM-1)){
            r_desc[i].tdes1 = 0x03000100;
            r_desc[i].tdes3 = (uint32_t)&r_desc[0];
        }
    }
    reg_write_32(DMA_BASE_ADDR+DmaRxBaseAddr,(uint32_t)&r_desc[0]);
